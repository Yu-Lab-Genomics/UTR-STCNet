...
class BatchConverter(object):
    ...
    def __call__(self, raw_batch: Sequence[Tuple[str, str]]): 
        ...
        seq_str_list, batch_labels = zip(*raw_batch)
        ...
        for i, (label, seq_str, seq_encoded) in enumerate(zip(batch_labels, seq_str_list, seq_encoded_list)):
            ...
            tokens[i, int(self.alphabet.prepend_bos) : len(seq_encoded)+ int(self.alphabet.prepend_bos)] = seq
...
class Alphabet(object):
    def __init__(
        ...
        prepend_toks: Sequence[str] = ("<pad>",), 
        append_toks: Sequence[str] = ("<unk>", "<cls>", "<mask>"), 
...
class UTRDATA(BaseDataset):
    def __init__(
        ...
        if utr:
            ...
            seq="ss"
        rawdata = RawData(root, seq=seq, label=label_class)
...
# alphabet = Alphabet(standard_toks={"A", "C", "G", "T"})
# file = '/pool1/liuzhouwu/datasets/UTR/muscle_merged_traintData.csv'
# dataset = UTRDATA(root=file, utr="UTR", label_class="te")
# batches = dataset.get_batch_indices(toks_per_batch = 4096, extra_toks_per_seq = 0)
# batches_loader = torch.utils.data.DataLoader(batches, 
#                                              batch_size = 1,
#                                              num_workers = 8,
#                                              shuffle = True
...
#                                             batch_sampler=[batch], 
#                                             shuffle = False)
#     for (labels, strs, tokens) in dataloader:
#         print(labels[0])
#         print(strs[0])
#         print(tokens[0])
